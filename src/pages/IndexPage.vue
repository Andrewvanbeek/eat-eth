<template>
  <q-page>
    <div class="bg-green-300 border-green-600 border-b p-4 m-4 rounded">
     ...
     <video id="video" width="720" height="560" ref="video" autoplay muted></video>
...
function loadLabeledImages() {
  const labels = ['Black Widow', 'Captain America', 'Captain Marvel', 'Hawkeye', 'Jim Rhodes', 'Thor', 'Tony Stark', 'Andrew']
  return Promise.all(
    labels.map(async label => {
      const descriptions = []
      for (let i = 1; i <= 2; i++) {
        const img = await faceapi.fetchImage(`http://localhost:8080/labeled_images/${label}/${i}.jpg`)
